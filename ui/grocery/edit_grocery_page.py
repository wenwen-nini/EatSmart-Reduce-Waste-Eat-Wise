...
from PIL import Image # IMPORT PIL FOR SUPPORTING IMAGES
from tkinter import messagebox as mb # IMPORTS MESSAGEBOX
from database.grocery_query import updateGrocery, getCategoryForSearch # IMPORTS NECESSARY QUERIES


def open_edit_grocery_page(parent, grocery_data, user_id, current_frame=None):
    ...
    display_area.grid_columnconfigure(0, weight=0)
    display_area.grid_columnconfigure(1, weight=1)

    # EDIT GROCERY LABEL
    edit_grocery_label = ctk.CTkLabel(
        display_area,
    ...
    edit_grocery_label.grid(row=0, column=0, columnspan=2, pady=10)

    def edit_grocery(user_id, category_var, categories):
        ...
        date = date_of_item_entry.get() # GETS THE DATE VALUE
        ...
        category_id = categories[category_name] # GETS THE ID OF CATEGORY SELECTED
        updateGrocery(user_id, grocery_data[0], grocery, quantity, category_id, date) # UPDATES THE GROCERY ITEM INTO THE GROCERY TABLE

    # GROCERY ITEM LABEL
    grocery_item = ctk.CTkLabel(
        display_area,
        text=grocery_data[1],
        font=("Arial Black", 16),
        text_color="#ead729",
    ...
    grocery_item.grid(row=1, column=0, padx=(100,0), pady=10, sticky="w")

    # GROCERY ENTRY
    grocery_entry = ctk.CTkEntry(
        display_area,
        placeholder_text="Enter the grocery item: ",
        font=("Arial Black", 14),
        height=50,
        width=350,
        corner_radius=12
    )
    grocery_entry.grid(row=1, column=1, padx=20, pady=10, sticky="w")

    # CATEGORY ITEM LABEL
    category_item = ctk.CTkLabel(
        display_area,
        text=grocery_data[3],
        font=("Arial Black", 16),
        text_color="#ead729",
    ...
    category_item.grid(row=2, column=0, padx=(100,0), pady=10, sticky="w")
    ...
    category_field_selector.grid(row=2, column=1, padx=20, pady=10, sticky="w")

    # QUANTITY ITEM LABEL
    quantity_item = ctk.CTkLabel(
        display_area,
        text=grocery_data[2],
        font=("Arial Black", 16),
        text_color="#ead729",
    ...
    quantity_item.grid(row=3, column=0, padx=(100,0), pady=10, sticky="w")
    ...
    quantity_entry.grid(row=3, column=1, padx=20, pady=10, sticky="w")

    # DATE ITEM LABEL
    date_of_item = ctk.CTkLabel(
        display_area,
        text=grocery_data[4],
        font=("Arial Black", 16),
        text_color="#ead729",
    ...
    date_of_item.grid(row=4, column=0, padx=(100,0), pady=10, sticky="w")

    # DATE ITEM ENTRY
    date_of_item_entry = ctk.CTkEntry(
        display_area,
    ...
    date_of_item_entry.grid(row=4, column=1, padx=20, pady=10, sticky="w")
    ...
    save_grocery_button = ctk.CTkButton(
        ...
        corner_radius=12,
        hover_color="#95E769",
        command=lambda: edit_grocery(user_id, category_variable, category_map) # CALLS THE FUNCTION FOR QUERY
    )
    save_grocery_button.grid(row=5, column=0, padx=(100,0), sticky="w")
    ...
    cancel_button.grid(row=5, column=1, padx=(0, 130), pady=10, sticky="e")

    # FOOTER FRAME
    footer_frame = ctk.CTkFrame(display_area, fg_color="white")
    footer_frame.grid(row=6, column=0, columnspan=2, pady=(3,0))
